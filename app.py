...
def load_image(file, max_size=900):
...
def build_profile(ref_imgs):
    feats = [features(img) for img in ref_imgs]
    pal_all = []
    for img in ref_imgs:
...
if st.button("Créer / Mettre à jour le profil feed"):
    if not ref_files or len(ref_files) < 5:
        ...
        refs = [load_image(f) for f in ref_files]
        st.session_state.profile = build_profile(refs)
...
if profile and cand_files:
    ...
    for f in cand_files:
        img = load_image(f)
        s = score(profile, img)
        results.append({"filename": f.name, **s})
    results.sort(key=lambda x: x["score"], reverse=True)

    st.markdown("### Top 9 (pré-sélection)")
    for r in results[:9]:
        st.markdown(f"**{r['label']} — {r['score']}/100**  \n{r['filename']}")
